"""
Overlay 热点队列：只增不减，按半衰期衰减热度。
- 数据存储：SQLite 表 overlay_queue(ts, symbol, heat, last_touch_ts)
- 接口：
    decay(half_life_hours)        # 全量衰减（按 ts 计算）
    bump(symbols, weight)         # 对一批 symbol 加热
    top(limit, min_heat)          # 取热度前 N
    update_from_t24(t24, k)       # 用 24h 涨跌幅绝对值 TopK 入队
"""
...
def decay(half_life_hours: float = 2.0) -> None:
    ...
    with _conn() as c:
        cur = c.execute("SELECT symbol, heat, ts FROM overlay_queue")
        rows = cur.fetchall()
        for sym, heat, ts in rows:
...
def top(limit: int = 18, min_heat: float = 0.01) -> List[str]:
    with _conn() as c:
        cur = c.execute("SELECT symbol FROM overlay_queue WHERE heat>? ORDER BY heat DESC LIMIT ?",
                        (float(min_heat), int(limit)))
        return [r[0] for r in cur.fetchall()]

def update_from_t24(t24: list, k: int = 30) -> None:
    """用 24h 绝对涨跌幅 TopK 做热点加热；不移除旧热点。"""
    try:
        sorted_items = sorted(
            t24,
            key=lambda x: abs(float((x.get("priceChangePercent") or "0").replace("%",""))),
            reverse=True
        )[: int(k)]
